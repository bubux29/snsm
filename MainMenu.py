...
import log
import formation_db
from liste_popup import PopupList
...
class ExpertMenuLieux(BoxLayout):
    lieu = ObjectProperty(None)
    description = ObjectProperty(None)
    popup_warning = ObjectProperty(None)
    popup_ok = ObjectProperty(None)

    def on_choix_lieu(self, index):
        print ("Voici l'index: " + self.nom_de_lieux[index])

    def __init__(self, **kwargs):
        super(ExpertMenuLieux, self).__init__(**kwargs)
    ...
    def check_on_valid(self):
        if not self.lieu.text or not self.description.text:
            pop_warn(self.popup_warning, "Veuillez compléter le formulaire")
            return False
        elif formation_db.ajouter_lieu_db(self.lieu.text, self.description.text):
            pop_warn(self.popup_warning, "Le lieu " + self.lieu.text + " est déjà défini en base de données")
            return False
        else:
            pop_ok(self.popup_warning, "Le lieu " + self.lieu.text + " a bien été ajouté à la base de données")
            clear_text(self.lieu)
            clear_text(self.description)
            return True
...
class ExpertMenuMain(BoxLayout):
    ...
    lieux_bouton = ObjectProperty(None)
    formateur_bouton = ObjectProperty(None)
    ...
    def bind_lieux(self,fun):
        self.swap_fun(self.lieux_layout)
    def creer(self, fun, lvlayout, lilayout):
        self.eleve_layout = lvlayout
        self.lieux_layout = lilayout
        self.swap_fun = fun
        self.eleves_bouton.bind(on_release=self.bind_eleve)
        self.lieux_bouton.bind(on_release=self.bind_lieux)
        return self
...
class ExpertMenuLayout(Widget):
    ...
    def creer(self):
        ...
        self.lieux = ExpertMenuSub(ExpertMenuLieux(), self.go_home)
        self.main = ExpertMenuMain().creer(self.swap_layout, self.eleves, self.lieux)
